...
@section('content')
    ...
    <div class="px-4 mb-8 space-y-6">
        {{-- Search Bar --}}
        <form action="{{ url()->current() }}" method="GET" class="relative max-w-2xl">
            <input type="text" name="search" value="{{ request('search') }}" 
                placeholder="Search inventory items..." 
                class="w-full h-14 bg-white dark:bg-slate-800 border-none rounded-2xl shadow-sm px-6 pr-12 text-logos-blue dark:text-white font-bold focus:ring-2 focus:ring-logos-gold transition-all">
            <button type="submit" class="absolute right-4 top-4 text-gray-400 hover:text-logos-blue">
        ...
        <div class="flex items-center space-x-2 overflow-x-auto pb-2 custom-scrollbar">
            @foreach($tabs as $tab)
                <a href="{{ route('storekeeper.dashboard', ['category' => $tab]) }}" 
                class="px-6 py-2.5 rounded-full whitespace-nowrap font-black text-xs uppercase tracking-widest transition-all
                {{ (request('category', 'ALL') == $tab) 
                    ? 'bg-logos-blue text-white shadow-lg shadow-blue-900/20' 
                    : 'bg-white dark:bg-slate-800 text-gray-400 hover:text-logos-blue border border-gray-100 dark:border-slate-700' }}">
                    {{ $tab }}
                </a>
            @endforeach
        </div>
